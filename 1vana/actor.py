...
class Actor:
    def __init__(self, x=0, y=0, ibi=0, ibp = [0,0], w=32, h=32, ib=""):
        ...
        self.bg_color          = env.WHITE
        self.draw = 0


    def imageload(self, source_file="", bg_color=env.WHITE):
    ...
    def put(self):
        ...
        if(pyxel.frame_count % env.ICON_ACTION_WAIT == 0):
        ...
        if(len(self.pixmap) > 0):
            ...
            for color in pixmap:
                ...
                if( pix_count != 0 and (x == 0 or p == 0) ):
                    ...
                    if( (y - self.position_y) > self.height or y > env.WINDOW_HEIGHT):
                        break
            ...
            pyxel.blt(self.position_x, self.position_y, self.img_bank_index, self.actions[self.frame][0], self.actions[self.frame][1], self.width, self.height, env.WHITE)
    ...
    def set_bg_color(self, bg_color=env.WHITE):
    ...
    def put2(self, bgcolor=env.WHITE):
        """
        アクターを指定済みの座標に表示する
        """
        # 登録されている画像やイメージバンクの範囲の数をカウントする
